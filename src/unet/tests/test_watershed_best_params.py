...
def test_watershed_json_to_eval_args_prints_h_maxima(tmp_path: Path) -> None:
    """INTENT: eval-args translator emits --watershed-h-maxima from watershed best JSON."""
    params = WatershedParamSet(5, 0, 1, 64, False, None, h_maxima=8)
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "unet.watershed_json_to_eval_args",
            str(json_path),
        ],
    ...
    assert "--watershed-h-maxima" in tokens
    assert tokens[tokens.index("--watershed-h-maxima") + 1] == "8"
