...
class SlurmScriptTests(unittest.TestCase):
    ...
    def test_yolo_train_script_rewrites_copied_dataset_yaml_to_tmpdir(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            uv_path.write_text(
                """#!/bin/bash
# Run real Python for the dataset YAML rewrite (stdin script): uv run python - <yaml> <dir>
...
# Stub training and any other uv run (no real train.py in this harness)
...
if [[ "$script" == "../evaluation/evaluate.py" ]]; then
  ...
  printf '{"sample":{"aji":0.4,"f1_iou50":0.5,"f1_iou75":0.45,"mF1_iou50_95":0.48}}' > "$output_json"
...
""",
    ...
    def test_eval_script_help_mentions_reusable_directory_and_config_inputs(
        ...
        self.assertIn("--watershed-tune-root", result.stdout)

    def test_eval_script_appends_watershed_args_when_config_has_tuning_json(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            temp_path = Path(tmpdir)
            ...
            shutil.copy(
                REPO_ROOT / "src" / "evaluation" / "watershed_json_to_eval_args.py",
                fake_repo / "src" / "evaluation" / "watershed_json_to_eval_args.py",
            )
...
""",
            ...
            ws_json = temp_path / "watershed_best_test.json"
            ws_json.write_text(
                json.dumps(
                    {
                        "best_params": {
            ...
            config_file = temp_path / "models.tsv"
            config_file.write_text(
                f"Custom\tcustom_model.keras\t1\t_PPL\t{ws_json}\n",
                encoding="utf-8",
            )
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            uv_calls = uv_log.read_text(encoding="utf-8")
            self.assertIn("--instance-method", uv_calls)
            self.assertIn("watershed", uv_calls)
            self.assertIn("--watershed-min-distance", uv_calls)
            self.assertIn("--watershed-connectivity", uv_calls)
            self.assertIn("3", uv_calls)
