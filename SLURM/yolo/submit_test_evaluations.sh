...
Manual one-off:
  sbatch --export=ALL,VARIANT='PPL+AllPPX' SLURM/yolo/run_sahi_test_eval.sh
EOF
    exit 0
fi
...
for variant in "${MICROSCOPY_VARIANTS[@]}"; do
    ...
    echo "Submitting run_sahi_test_eval (SAHI) variant=$variant mem=$SAHI_MEM time=$SAHI_TIME"
    sbatch \
        --mem="$SAHI_MEM" \
        --time="$SAHI_TIME" \
