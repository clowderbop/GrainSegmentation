...
def instance_overlap_stats(
    ...
    gt_ids = _instance_ids(true_instances)
...
def _iou_matrix_from_overlap(stats: OverlapStats) -> np.ndarray:
    ...
    for tid, pid, inter in zip(
        ...
        mat[i, j] = _iou_from_intersection(
...
def compute_merged_view_pq(
    ...
    if nt == 0 and np_ == 0:
        ...
        iou_matrix = _iou_matrix_from_overlap(stats)
